...
if __name__ == "__main__":
    ...
    for to_address_dict in to_address_list:
        ...
        if not LSO_flag:
            print(
                f"[!] Unable to pull LSO ECommerce or GroundBasic rate for {toAddress.name}"
            )
        ...
        print(
            f"[+] Chosen rate for purchase for {toAddress.name}: {chosen_rate['carrier']} {chosen_rate['service']}"
        )
        if READY_TO_BUY:
            try:
