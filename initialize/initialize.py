...
def calc_parcel_dict(order_quantity):
    ...
    weight = str(
        int(math.ceil((parcel_weight + chikfu_weight + ice_pack_weight) / num_parcels))
    )
    parcel_dict = {
        "length": dimensions,
        "width": dimensions,
...
class Initialize:
    ...
    def initialize_to_address_squarespace(self):
        for file_name in os.walk("../Orders"):
            if (
                file_name[2][0][:4] == "done"
                or os.path.splitext(file_name[2][0]) != ".json"
            ):
                continue
            ...
            os.rename(
                os.path.join(file_name[0], file_name[2]),
                os.path.join(file_name[0], new_filename),
            )
